...
import {
  ...
  transactionsColumns,
  categoriesColumns,
  categoryBudgetsColumns,
} from '../data/queryColumns.js';
...
controller.getTransactions = (req, res, next) => {
  ...
  query(
    `SELECT ${transactionsColumns} FROM transactions t INNER JOIN categories c on t.category_id = c._id WHERE t.user_id = ${userID}; `
  )
    .then((query) => {
      // console.log(query.rows);
...
controller.getCategories = (req, res, next) => {
  ...
  query(
    `SELECT ${categoriesColumns} FROM categories c WHERE c.user_id = ${userID}; `
  )
    .then((query) => {
      // console.log(query.rows);
...
controller.getCategoryBudgets = (req, res, next) => {
  ...
  query(
    `select ${categoryBudgetsColumns} from categorybudgets b INNER JOIN categories c ON b.category_id = c._id WHERE c.user_id=${userID}; `
  )
    .then((query) => {
      // console.log(query.rows);
