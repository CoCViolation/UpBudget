...
controller.getTransactions = (req, res, next) => {
    ...
    query(`SELECT t.accountType, t.accountFrom, t.accountTo, t.account, t.date, t.payee, t.notes, t.amount, t.debit, t.credit, c.name FROM transactions t INNER JOIN categories c on t.category_id = c._id WHERE t.user_id = ${userID}; `)
        .then(query => {
            // console.log(query.rows);
...
controller.getCategories = (req, res, next) => {
    ...
    query(`SELECT c.name FROM categories c WHERE c.user_id = ${userID}; `)
        .then(query => {
            // console.log(query.rows);
...
controller.getCategoryBudgets = (req, res, next) => {
    ...
    query(`select b.dateFrom, b.dateTo, b.amount, c.name from categorybudgets b INNER JOIN categories c ON b.category_id = c._id WHERE c.user_id=${userID}; `)
        .then(query => {
            // console.log(query.rows);
