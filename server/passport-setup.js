...
passport.use(
  new GoogleStrategy(
    ...
    function (accessToken, refreshToken, profile, done) {
      ...
      query(`SELECT * FROM users WHERE oauthkey='${String(googleID)}';`)
        .then((queryData) => {
          ...
          //else create
            ...
            query(
              `INSERT INTO users (name, oauthkey) VALUES ('${name}', '${String(
                googleID
              )}');`
            )
              .then((query) => {
              ...
              .then(async () => {
                await query(
                  `SELECT * FROM users WHERE oauthkey='${String(googleID)}';`
                ).then((query) => {
                  // console.log(query)
